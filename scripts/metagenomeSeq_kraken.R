...
# Check microbiome samples with sparse features
microbiome_sparseSamples = which(colSums(MRcounts(microbiome) > 0) < 2) ## just counts how many rows have less than 2 hits
if (length(microbiome_sparseSamples) == 0) {
  print("No microbiome Samples removed")
} else {
  microbiome = microbiome[, -microbiome_sparseSamples]
  print(paste0(length(microbiome_sparseSamples)," Samples removed. Listed below:"))
  print(names(microbiome_sparseSamples))
}
