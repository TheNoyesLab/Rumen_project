...
amr_sparseSamples = which(colSums(MRcounts(amr) > 0) < 2) ## just counts how many rows have less than 2 hits
if (length(amr_sparseSamples) == 0) {
  print("No AMR samples removed")
} else {
  amr = amr[, -amr_sparseSamples]
  print(paste0(length(amr_sparseSamples)," Samples removed for having < 2 features. Listed below:"))
  print(names(amr_sparseSamples))
}
