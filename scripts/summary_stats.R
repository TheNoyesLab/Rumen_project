...
gene_counts = amr_melted_raw_analytic[Level_ID=='Gene',]
...
## By ID
ID_amr_percentage <- gene_counts[,.(class_sum = sum(Normalized_Count), class_mean = mean(Normalized_Count)), by= .(ID) ][, total := sum(class_sum)][,percentage := class_sum/ total * 100, by = .(ID)]
ID_amr_percentage
ID_amr_percentage[,.(mean_sum_by_sample = mean(class_sum))]


## Total AMR abundance by Class
AMR_counts_Class <- amr_melted_raw_analytic[Level_ID == "Class",.(class_sum = sum(Normalized_Count), mean_class = mean(Normalized_Count), sd_class = sd(Normalized_Count)), by= .(Name) ][,total := sum(class_sum)][,percentage := class_sum/ total * 100, by = .(Name)]
AMR_counts_Class

## Total AMR abundance of each class, by Study
AMR_counts_by_Study <- amr_melted_analytic[Level_ID == "Class",.(class_sum = sum(Normalized_Count), mean_class = mean(Normalized_Count), sd_class = sd(Normalized_Count)), by= .(Name, Study) ][,total := sum(class_sum), by = .(Study)][,percentage := class_sum/ total, by = .(Study)]
AMR_counts_by_Study
...
fig1 <- ggplot(amr_melted_raw_analytic[Level_ID=='Group'][Normalized_Count > 0 , .(sample_count = .N), by = .(Name, Study) ],
       aes(Study, Name , label=sample_count)) +
  geom_tile(aes(fill = sample_count), color = "white") +
  geom_text() + 
  scale_fill_gradient(low = "grey", high = "steelblue") +
  ylab("List of ICEberg accessions") +
  xlab("Study") +
...
ggsave("Figure9_ICEberg_gene_count.jpeg", width = 30, height = 20, units = "cm")
